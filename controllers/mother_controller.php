	...
	class MotherCtrl {
		...
		*/
		protected function _sendMail(){
			...
			$this->_objMail->Mailer 	= $_ENV['MAIL_MAILER'];
			...
			$this->_objMail->SMTPAuth   	= $_ENV['MAIL_SMTPAuth'];
			$this->_objMail->SMTPSecure 	= $_ENV['MAIL_SMTPSecure'];
			$this->_objMail->Port       	= $_ENV['MAIL_PORT'];
			$this->_objMail->Host       	= $_ENV['MAIL_HOST'];
			$this->_objMail->Username 		= $_ENV['MAIL_USERNAME'];
			$this->_objMail->Password 		= $_ENV['MAIL_PWD'];

			// Comment envoyer le mail
			$this->_objMail->IsHTML($_ENV['MAIL_ISHTML']); // en HTML
			$this->_objMail->setFrom($_ENV['MAIL_FROM_EMAIL'], $_ENV['MAIL_FROM_NAME']); // Expéditeur
